...
This developer journey is intended for anyone who wants to experiment, learn, enhance and implement a new method for Predicting Equipment failure using IoT Sensor data. IoT sensors mounted on devices like the Robot arms in automated manufacturing systems and the Process monitoring and Control equipment collect and transmit data on a continuous basis which is Time stamped.
...
Once, a Change point is detected in one key operating parameter of the IoT equipment, then it makes sense to follow it up with a Test to predict if this recent shift will result in a failure of an equipment.  This journey is an end to end walk through of a Prediction methodology that utilizes multivariate (involving multiple variables) IoT data and [Logistic Regression](https://en.wikipedia.org/wiki/Logistic_regression) to predict any failure of an equipment. Predictive packages in Python 2.0 software are used in this Journey along with sample Sensor data loaded into the Data Science experience cloud.
  
All the intermediary steps are modularized and the code is open sourced (under Apache 2.0 license) to enable developers to use / modify the modules / sub-modules as they see fit in their specific application.
  
When you have completed this journey, you will understand how to
  
*	Read IoT Sensor data stored in a database
*	Configure the features and target variables for the Prediction model
*	Split the multivariate data into Train and Test datasets
...
1.	Reading IoT Sensor data from database
